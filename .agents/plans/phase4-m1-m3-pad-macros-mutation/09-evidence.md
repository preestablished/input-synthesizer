...
| 05 M1 mixer/server/goldens | Satisfied | 50 propose-level goldens (`testdata/golden/m1/`), transport-free replay path, bench on both arches. Gap closed this round: `http_shell_serves_healthz_and_metrics` (tests/http_shell.rs) + `generator_unavailable_metric_increments_on_degradation` (tests/grpc_integration.rs). Deferred (beads): k=256 golden fixture; propose-level macro-mix goldens (testdata changes ⇒ version-bump-coupled). |
